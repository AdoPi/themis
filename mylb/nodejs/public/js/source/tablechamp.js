    ...
    function scoringAdd() {
        ...
        if (isDoubles) {
            ...
            if (parseInt(t1s) > parseInt(t2s)) {
                ...
                $.ajax('/players/addscore/'+n1+'/'+3);
            } else {
                if (parseInt(t1s) == parseInt(t2s)) {
                    // draw
                    var n1 = localData.playersByKey[t1p1Key].name;
                    var n2 = localData.playersByKey[t2p1Key].name;
                    $.ajax('/players/addscore/'+n1+'/'+1);
                    $.ajax('/players/addscore/'+n2+'/'+1);
    ...
    function sidebarInitPlayerEvents() {
        ...
        $('.players-add form').off('click').on('submit', function() {
            ...
            $.each( playersField.val().split('\n'), function( index, player ){
                ...
                $.ajax('/players/add/'+player+'/').done( function() {
                    playerSettingsUpdate();
